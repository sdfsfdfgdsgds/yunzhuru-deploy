        ...
        const categories = ref([
            {
                ...
                items: [
                  ...
                  {
                    title: '如何选择注模式',
                    content: '默认为链路注入，若注入后无法运行或者卡启动页，可尝试入口反射；DCloud/UniApp/受保护Manifest等特殊壳包请选择保资源注入；如果注入后没有弹窗效果，可尝试组件工厂模式注入',
                    searchKeywords: ''
                  },
                  {
                    title: '不同的注入模式都有什么用',
                    content: '入口继承(代表由壳先启动,然后启动原入口,但是部分应用会检测入口导致闪退)。入口反射(和入口继承一样,不过是反射调用原入口,通常不会被检测到)。保资源注入(入口反射+保留原APK资源和主DEX,只追加壳DEX/so,适合DCloud/UniApp/特殊壳包)。组件工厂(执行优先级最高,是入口动态劫持的一种)。基类链路(由原本的程序先启动,壳再启动,比较好隐藏壳链,但是对加固混淆的应用无效,会编译失败)',
                    searchKeywords: ''
                  },
                  {
                    title: '无法删除注入任务',
